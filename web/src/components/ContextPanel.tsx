...
import { Plus, X, Sparkles, Crosshair, Scale, Gamepad2, ChevronLeft, ChevronRight, Skull, Check } from "lucide-react";
...
export function ContextPanel({
  ...
  // stepper: one concern per step; Malédictions only exists under Deep of Night.
  const hasMalus = form.don >= 1 && meta.curses.length > 0;
  const steps = [
    { key: "cible", label: "Cible", icon: Crosshair },
    { key: "style", label: "Objectif & style", icon: Gamepad2 },
    ...(hasMalus ? [{ key: "malus", label: "Malédictions", icon: Skull }] : []),
  ];
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/* stepper nav — every step is clickable, so re-running is never a linear slog */}
        ...
        {steps.map((s, i) => {
          ...
          return (
            <button
              ...
              onClick={() => setStep(i)}
              className={cn(
                "flex flex-1 items-center justify-center gap-1.5 border px-2 py-2 text-[11.5px] leading-tight transition",
                active ? "border-gold/60 bg-gold/10 text-gold-bright" : "border-line/45 bg-night-800/40 text-silver/70 hover:border-line-bright hover:text-ink"
              )}
            >
              <span className={cn("flex h-[18px] w-[18px] flex-none items-center justify-center rounded-full text-[9.5px] font-semibold", active ? "bg-gold/25 text-gold-bright" : "bg-night-700 text-silver/55")}>{i + 1}</span>
        ...
        {curKey === "style" && (
          <>
            <Section icon={Scale} title="Objectif">
              ...
              <div className="mt-2.5 text-center text-[12.5px] text-silver/75">Curseur : {weightHint}.</div>
            </Section>

            <Section icon={Gamepad2} title="Style de jeu">
              <Field label="Profil de jeu">
                <p className="mb-2.5 text-[12px] leading-snug text-silver/70">Les buffs « par action » ne comptent que pour les actions déclarées.</p>

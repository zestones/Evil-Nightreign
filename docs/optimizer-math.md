...
- **(INV-3, verified $v \ge 0$ on non-debuff effects)** On the owned collection no *buff* lowers a tracked multiplicative field: there is no `*AttackRate` $< 1$ and no `*DamageCutRate` $> 1$ on any non-debuff relic effect, deep included. Nonnegativity is a **verified data invariant, asserted at load time** for non-debuff keys. Deep-of-Night **curses** deliberately break it — they *are* the malus (`*AttackRate` $< 1$, `*DamageCutRate` $> 1$, `maxHpRate` $< 1$, `add*Status` $< 0$). Since each curse is inseparable from its buff (same relic) and the log representation is sign-agnostic, they enter as **signed** contributions $v(e)$ that may be $< 0$: the only load-bearing guard is the log domain $v > 0$ (multiplier), the sign is carried by the monotone $\exp$ downstream. Consequence: monotonicity (Prop. 1) is lost on cursed pools — adding a cursed relic can lower an axis — so $\mathrm{Agg}$'s codomain is $\mathbb{R}$, not $\mathbb{R}_{\ge 0}$. The greedy $\tfrac12$-bound (§4) no longer holds there, but the search already rests on **beam + exhaustive verification** (§5.3), and `dominates` iterates the **key union** so a cursed dominator cannot falsely prune a clean relic (Theorem 2 stays sound).
...
1. $\text{base}_\tau$ is treated as **constant in $A$**: stat-scaling effects ($+\text{Str} \Rightarrow$ higher AR) are excluded from this analysis and handled at search time (§6.5). *Multi-source extension (2026-07-17):* each damage SOURCE (weapon attack, spell cast, character skill/ultimate — `engine/sources.py`) contributes its **own constant base** $\text{base}_s$; the relic side stays inside the same per-key $\mathrm{Agg}$, so nothing below changes. The catalyst stat-feedback ($+\text{Int} \Rightarrow$ higher spell base) is a **second instance of the §6.5 coupling**, handled by the same memoized fixed point (`Scorer._source_power` shares `_ar`'s cache key) — no new proof obligation.
...
$$\mathrm{OFF}(R) \;=\; \sum_{a} p_a \cdot D\!\Big(\mathrm{base}_{s(a)} \cdot \exp\big(\textstyle\sum_{c \in C(a)} \mathrm{Agg}_c(R)\big)\Big),$$

with $D$ the damage pipeline and $\mathrm{base}_{s(a)}$ the attack-power base of action $a$'s SOURCE — the equipped weapon's $\mathrm{AR}\times\mathrm{MV}(a)$ for weapon-family actions, the resolved spell/skill base for sourced actions (`engine/sources.py`; a declared cast with **no** resolved catalyst spell has $\mathrm{base}=0$, never a silent weapon-AR fallback). Three consequences. (i) Per action, the objective is still $D(\text{base} \cdot e^{\text{submodular}})$; the mixture over actions — now a weighted sum of exponentials with **per-source bases** — stays monotone, not submodular: exactly the regime §4 already assigns to beam $+$ exhaustive verification, no new machinery (re-verified empirically post-extension: beam($k{=}12$) $=$ exhaustive on a mixed sorcery/melee Recluse context, `tests/test_sources.py` companion check). (ii) Theorem 2 is untouched: profile dimensions refine from $(\text{atk}, \text{type})$ to $(\text{atk}, \text{type}, \text{class})$ and the proof never counts dimensions — **Proposition 1 and Theorem 2 hold unchanged as long as the relic aggregation stays per-key log-additive**, which no source touches. (iii) Under the default profile a crit-only or throwing-knife-only relic is worth exactly nothing — matching the measured game behavior instead of the $\times 8.5$ fantasy multipliers an ungated model produces.

### 2.2 Status ailments

Every hit adds the weapon's buildup (SpEffect `bloodAttackPower`, … — chain validated on Reduvia: $45$/hit) plus the relic on-hit buildup (aggregated per key like everything else, dimension $(\text{stbuild}, s)$) to the target's gauge, which procs at its resistance threshold.

**The threshold escalates after every proc** (`ResistCorrectParam`, per boss, extracted into `nightlords.json`). Per the game's own field annotations the gauge *resets* after a proc, and after $N$ activations the fill target becomes $\text{base}\cdot\text{addRate}_N + \text{addPoint}_N$; the cumulative buildup to the $N$-th proc is the running sum of those per-proc fills. For Gladius bleed (base $252$, addPoint $290, 747, \dots$) the cumulative thresholds are $252, 794, 1793, \dots$, i.e. the procs land at $\sim 5.6, 17.6, 39.8$ hits (at $45$/hit). This is derived from the field definitions, and *independently* matches the player's rough "first proc ~5 hits, second ~20". So bleed is *front-loaded*: a full fight yields only $\sim 3$ procs, not one every $5.6$ hits. A steady $\text{buildup}/\text{resistance}$ rate overcounts it $\sim 3\times$ and wrongly ranked bleed daggers over the holy Black Knife vs the holy-weak Gladius; the escalation model corrects that (holy now wins, matching play experience).

Expected status damage is therefore amortized over the fight: with $H = \text{HP}/\text{direct-dmg-per-hit}$ the estimated hit count, $n_s = \#\{k : \text{threshold}_k \le H\cdot\text{buildup}_s\}$ procs land, contributing per hit

$$\text{dmg/hit} \mathrel{+}= \sum_{s} \frac{n_s \cdot \text{proc}_s(\text{HP}_{\text{target}})}{H}$$

Two proc shapes, both escalation-aware:

- **Burst** (bleed, frost): each proc is a one-shot % of max HP, so fight damage $= n_s \cdot \text{burst}_s$. **bleed** $= 15\%$ — *locked* by the player-measured $371$ proc (day-3 troll) and the engine's canonical hemorrhage row (SpEffect $1612$); **frost** $= 5\%$ (SpEffect $1611$, structural twin — to confirm).
- **DoT** (poison, rot): a proc applies a damage-over-time that later procs *refresh* rather than stack, so once the first proc lands the DoT is up until its last application's duration lapses or the fight ends. Fight damage $= \text{uptime}_{\text{s}} \cdot \text{rate}_s$, with $\text{uptime}_{\text{s}} = \min(\text{fight}_{\text{s}}, \text{last-proc}_{\text{s}} + \text{dur}_s) - \text{first-proc}_{\text{s}}$ and hits$\leftrightarrow$seconds via the weapon's class cadence. This replaces an earlier per-proc$\times 0.5$ approximation that *double-counted* overlapping DoTs (poison on a boss: physically $\le \text{rate}\times\text{fight}$, but the old model summed a half-DoT per proc). **poison** $= 40$s at $1.3$s ticks, $10 + 0.10\%$ HP/tick; **rot** $= 40$s, $15 + 0.15\%$ HP/tick (structural, from the stateInfo-$2$/-$5$ SpEffect rows).

Resistance $999$ = immune. Sleep/madness/curse carry no damage term (control/niche). Deep-of-Night HP scaling applies to the proc values. Frost additionally applies **damage taken $\times 1.15$ for $30$s** (a $195$-row SpEffect family, stateInfo $260$) — near-permanent uptime once procced, so a frost weapon multiplies the whole build's damage against non-immune targets. Only bleed's proc value is player-validated; frost/poison/rot values are structural (from the params).

...
Every hit adds the weapon's buildup (SpEffect `bloodAttackPower`, … — chain validated on Reduvia: $45$/hit) plus the relic on-hit buildup (aggregated per key like everything else, dimension $(\text{stbuild}, s)$) to the target's gauge, which procs at its resistance threshold. Expected damage per hit is therefore linear and slots directly into $D$:

$$\text{dmg/hit} \mathrel{+}= \sum_{s} \frac{\text{buildup}_s}{\text{resistance}_s(\text{target})} \cdot \text{proc}_s(\text{HP}_{\text{target}})$$

Proc values (`resources/statuses.py`): **bleed** $= 15\%$ max HP — *locked* by a player-measured $371$ proc matching a day-3 troll at $0.1\%$ and the engine's canonical hemorrhage row (SpEffect $1612$); **frost** $= 5\%$ (SpEffect $1611$, the structural twin row — to confirm); **poison / scarlet rot** $=$ DoTs read off their stateInfo-$2$/-$5$ rows ($\sim 31$ ticks of $10 + 0.10\%$ / $15 + 0.15\%$ HP). Resistance $999$ = immune. Sleep/madness/curse carry no damage term (control/niche). Deep-of-Night HP scaling applies to the proc values. DoT procs are credited at $50\%$ of their total (re-procs *refresh* the effect and waste the remaining duration, and fights truncate it — the exact overlap needs attack-cadence data); bursts pay in full. Frost additionally applies **damage taken $\times 1.15$ for $30$s** (a $195$-row SpEffect family, stateInfo $260$) — with a proc every $5$–$10$ hits the debuff is near-permanently up, so a frost weapon multiplies the whole build's damage against non-immune targets.
